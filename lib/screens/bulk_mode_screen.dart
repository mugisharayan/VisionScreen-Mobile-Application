...
class _BulkBoundingBox extends CustomPainter {
  ...
  @override
  bool shouldRepaint(_BulkBoundingBox o) => o.size != size;
}
...
class _BulkTumblingE extends CustomPainter {
  ...
  @override
  bool shouldRepaint(_BulkTumblingE o) => o.color != color;
}
...
class _BulkModeScreenState extends State<BulkModeScreen> {
  ...
  void _advanceStaircase(bool passed) {
    if (_jumpPhase) {
      if (passed) {
        ...
        _jumpIndex++;
        if (_jumpIndex >= _staircaseJumps.length) {
  ...
  bool get _needsReferral {
    ...
    final results = _eyeResults
        .map((r) => double.tryParse(r['logmar'] as String) ?? 0.0)
        .toList();
  ...
  Future<void> _saveFacility() async {
    ...
    if (facility.isEmpty || _currentScreeningId == null) return;
  ...
  Future<void> _saveAndShowResult() async {
    ...
    for (final r in _eyeResults) {
      if (r['eye'] == 'OD') {
        odLogmar = r['logmar'];
        odCantTell = r['cantTell'];
        odDuration = r['duration'];
      }
      if (r['eye'] == 'OS') {
        osLogmar = r['logmar'];
        osCantTell = r['cantTell'];
        osDuration = r['duration'];
      }
    }
    final outcome = _needsReferral ? 'refer' : 'pass';
    final screeningId = await ScreeningRepository.instance.insertScreening({
    ...
    if (widget.existingCampaignId != null) {
      ...
      CampaignRepository.instance
          ...
          .then((rows) {
            if (!mounted) return;
            final screened = rows
                .where((r) => r['outcome'] != null && r['outcome'] != 'pending')
                .length;
            setState(() {
          ...
          child: Row(
            children: [
              ...
              if (_section == 2) ...[
                ...
                Text(
                  _section == 4 ? 'Cover Eye Reminder' : 'Bulk Screening',
                  style: GoogleFonts.plusJakartaSans(
                    fontSize: 14,
                    fontWeight: FontWeight.w700,
                    color: Colors.white,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            'Fill in the campaign details before screening begins.',
            style: GoogleFonts.inter(
              fontSize: 13,
          ...
          GridView.count(
            ...
            children: _targetGroups.map((g) {
              ...
              return GestureDetector(
                onTap: () =>
                    setState(() => _targetGroup = g['label'] as String),
                          ...
                          children: [
                            Text(
                              ...
                              style: GoogleFonts.inter(
                                fontSize: 12,
                                fontWeight: FontWeight.w700,
                                color: active ? _teal : const Color(0xFF1A2A3D),
                              ),
                            ),
                            Text(
                              g['sub'] as String,
              ...
              child: _saving
                  ...
                  : Column(
                      children: [
                        ...
                        const SizedBox(height: 6),
                        Text(
                          'Tap to begin — patients will be added one by one',
                          style: GoogleFonts.inter(
                            fontSize: 11,
  ...
  Future<void> _registerAndProceed() async {
    ...
    // ── Duplicate detection ──────────────────────────────────
    final duplicates = await PatientRepository.instance.findPotentialDuplicates(
      name: name,
      age: _quickAge,
    ...
    if (duplicates.isNotEmpty && mounted) {
      final proceed = await showDialog<bool>(
        context: context,
        builder: (_) =>
            _BulkDuplicateDialog(newName: name, duplicates: duplicates),
      );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '$_patientCount screened · $_totalInCampaign registered · ${_campaignNameCtrl.text}',
                        style: GoogleFonts.inter(
                          fontSize: 11,
                          color: const Color(0xFF8FA0B4),
                        ),
          ...
          const SizedBox(height: 4),
          Text(
            'Quick 3-field registration — takes under 10 seconds.',
            style: GoogleFonts.inter(
              fontSize: 13,
              color: const Color(0xFF8FA0B4),
            ),
          ...
          Container(
            ...
            child: Row(
              children: [
                GestureDetector(
                  onTap: () =>
                      setState(() => _quickAge = (_quickAge - 1).clamp(1, 120)),
                  child: Container(
                    width: 36,
                    height: 36,
                    decoration: BoxDecoration(
                ...
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Column(
                      children: [
                        Icon(
                          g == 'M' ? Icons.male_rounded : Icons.female_rounded,
                          size: 24,
                          color: active ? _teal : const Color(0xFF8FA0B4),
                        ),
                        const SizedBox(height: 4),
                    ...
                    return CustomPaint(
                      ...
                      child: Padding(
                        ...
                        child: RotatedBox(
                          ...
                          child: CustomPaint(
                            size: Size(s * 0.8, s),
                            painter: _BulkTumblingE(color: _ink),
                          ),
                        ),
                      ),
                    );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 24),
          Text(
            eye == 'OD' ? 'Cover Left Eye' : 'Cover Right Eye',
            style: GoogleFonts.plusJakartaSans(
              fontSize: 22,
              fontWeight: FontWeight.w800,
              color: const Color(0xFF1A2A3D),
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 12),
                  Text(
                    'Begin ${eye == 'OD' ? 'Right' : 'Left'} Eye Test',
                    style: GoogleFonts.plusJakartaSans(
                      fontSize: 16,
                      fontWeight: FontWeight.w800,
                      color: Colors.white,
  ...
  Widget _buildResultPlaceholder() {
    ...
    final odResult = _eyeResults.firstWhere(
      (r) => r['eye'] == 'OD',
      orElse: () => {},
    );
    final osResult = _eyeResults.firstWhere(
      (r) => r['eye'] == 'OS',
      orElse: () => {},
    );
    final odSnellen = odResult.isNotEmpty
        ? VisualAcuity.toSnellen(odResult['logmar'] as String)
        : '—';
    final osSnellen = osResult.isNotEmpty
        ? VisualAcuity.toSnellen(osResult['logmar'] as String)
        : '—';
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: Icon(
                    passed ? Icons.check_rounded : Icons.warning_rounded,
                    color: col,
                    size: 36,
                  ),
                ),
                ...
                Text(
                  passed
                      ...
                      : 'Vision below threshold — referral needed',
                  style: GoogleFonts.inter(
                    fontSize: 12,
                    color: const Color(0xFF8FA0B4),
                  ),
                  ...
                  children: [
                    Expanded(
                      child: _vaCard(
                        'OD',
                        odSnellen,
                        odResult.isNotEmpty ? odResult['logmar'] as String : '',
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: _vaCard(
                        'OS',
                        osSnellen,
                        osResult.isNotEmpty ? osResult['logmar'] as String : '',
                      ),
                    ),
                  ],
                ),
          ...
          if (!passed) ...[
            ...
            GestureDetector(
              onTap: () async {
                ...
                if (_facilityCtrl.text.trim().isNotEmpty ||
                    _selectedFacility != null) {
                  await _saveFacility();
                }
                // Get patient data for referral letter
                final patient = await PatientRepository.instance.getPatient(
                  _currentPatientId!,
                );
  ...
  Widget _buildSummaryPlaceholder() {
    ...
    final referred = _sessionPatients
        .where((p) => p['outcome'] == 'refer')
        .length;
    final pending = _sessionPatients
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    _summaryStatCard(
                      pending > 0 ? '$pending' : '$passRate%',
                      pending > 0 ? 'Pending' : 'Pass Rate',
                      pending > 0 ? _amber : _teal3,
                    ),
                  ],
                ),
              ],
          ...
          const SizedBox(height: 4),
          Text(
            '$total patient${total == 1 ? '' : 's'} in this session',
            style: GoogleFonts.inter(
              fontSize: 12,
              color: const Color(0xFF8FA0B4),
            ),
          ...
          if (_sessionPatients.isEmpty)
            ...
            ..._sessionPatients.asMap().entries.map((e) {
              ...
              final initials = name
                  ...
                  .take(2)
                  .join();
              final col = outcome == 'pass'
                  ? _green
                  : outcome == 'refer'
                  ? _red
                  : _amber;
              final badgeLabel = outcome == 'pass'
                  ? 'Pass'
                  : outcome == 'refer'
                  ? 'Refer'
                  : 'Pending';
              ...
              return Container(
                margin: EdgeInsets.only(
                  bottom: i < _sessionPatients.length - 1 ? 10 : 0,
                ),
                padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(
                  color: Colors.white,
...
class _BulkDuplicateDialog extends StatelessWidget {
  const _BulkDuplicateDialog({required this.newName, required this.duplicates});

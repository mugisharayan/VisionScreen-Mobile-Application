...
const _missingVaValue = 'Not tested';
...
String _normaliseVisualAcuity(String? value) {
  final trimmed = value?.trim() ?? '';
  if (trimmed.isEmpty || trimmed == '�') {
    return _missingVaValue;
  }
  return trimmed;
}

String _patientDemographics(_Patient patient) =>
    '${patient.age} yrs | ${patient.gender} | ${patient.village}';

String _buildPatientWhatsAppMessage(_Patient patient) {
  final status = patient.referralStatus?.toUpperCase() ?? patient.outcome.toUpperCase();
  final subject = patient.outcome == 'refer' ? 'Referral' : 'Patient';
  return '''
*VisionScreen $subject Update*

*Patient:* ${patient.name}
*Demographics:* ${_patientDemographics(patient)}
*Facility:* ${patient.facility ?? 'N/A'}
*Status:* $status

*Visual Acuity*
- OD (Right Eye): ${patient.od}
- OS (Left Eye): ${patient.os}
- OU (Both Eyes): ${patient.ou}
...
Future<void> _exportPatientReport(
...
class _PatientsScreenState extends State<PatientsScreen> {
  ...
  Future<void> _loadPatients() async {
    ...
    // Use the repository for all patients. Pagination is still capped here
    // because this screen builds an in-memory filtered list.
    final allPatients = await PatientRepository.instance.getPatients(
      pageSize: 500,
    );
    final latestByPatient = await ScreeningRepository.instance
        .getLatestScreeningsForPatients(
    ...
    for (final r in patientRows) {
      ...
      final latest = latestByPatient[pid];
      ...
      list.add(
        _Patient(
          ...
          ageGroup: age < 18
              ...
              ? 'elderly'
              : 'adult',
          od: _normaliseVisualAcuity(od),
          os: _normaliseVisualAcuity(os),
          ou: _normaliseVisualAcuity(ou),
          outcome: outcome,
          ...
          facility: (latest?['referral_facility'] as String?)?.let(
            (f) => f.isEmpty ? null : f,
          ),
          referralStatus: (latest?['referral_status'] as String?)?.let(
            (s) => s.isEmpty ? null : s,
          ),
          campaignId: null,
    ...
    final campPatientRows = allPatients
        .where(
          (r) =>
              r['campaign_id'] != null &&
              (r['campaign_id'] as String).isNotEmpty,
        )
        .toList();
    final campList = <_Patient>[];
    for (final r in campPatientRows) {
      final age = (r['age'] as int?) ?? 0;
      final latest = latestByPatient[r['id'] as String];
      final outcome = (latest?['outcome'] as String?) ?? 'pending';
      campList.add(
        _Patient(
          initials: (r['name'] as String)
          ...
          ageGroup: age < 18
              ...
              ? 'elderly'
              : 'adult',
          od: _normaliseVisualAcuity(latest?['od_snellen'] as String?),
          os: _normaliseVisualAcuity(latest?['os_snellen'] as String?),
          ou: _normaliseVisualAcuity(latest?['ou_near_snellen'] as String?),
          outcome: outcome,
          date: latest?['screening_date'] != null
              ? _formatDate(latest!['screening_date'] as String)
              : 'Not screened',
          id: r['id'] as String,
          phone: (r['phone'] as String?) ?? '',
          facility: (latest?['referral_facility'] as String?)?.let(
          ...
          campaignId: (r['campaign_id'] as String?)?.let(
            (c) => c.isEmpty ? null : c,
          ),
  ...
  Widget build(BuildContext context) {
    ...
    // Campaign stats
    final campTotal = _campaigns.fold<int>(
      0,
      (sum, c) => sum + ((c['total'] as int?) ?? 0),
    );
    final campPassed = _campaigns.fold<int>(
      0,
      (sum, c) => sum + ((c['passed'] as int?) ?? 0),
    );
    final campReferred = _campaigns.fold<int>(
      0,
      (sum, c) => sum + ((c['referred'] as int?) ?? 0),
    );
    // Campaign pending = patients in campaigns with no screening yet
    final campPending = _allCampaignPatients
        .where((p) => p.outcome == 'pending')
        .length;
    // Combined
          ...
          Expanded(
            child: _loading
                ...
                : RefreshIndicator(
                    ...
                    child: () {
                      if (_query.isEmpty) {
                        // No search: show campaigns and individual patients.
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(context);
              await CampaignRepository.instance.deleteCampaign(
                c['id'] as String,
              );
              await _loadPatients();
              if (mounted)
  ...
  Future<void> _exportPatientToPDF(_Patient p) async {
    await _exportPatientReport(context, p, closeSheet: true);
  }
  ...
  Future<void> _shareToWhatsApp(_Patient p) async {
    final message = _buildPatientWhatsAppMessage(p);
...
class _PatientHistorySheetState extends State<_PatientHistorySheet> {
  ...
  Widget _historyRow(Map<String, dynamic> r, int index, int total) {
    ...
    final od = _normaliseVisualAcuity(r['od_snellen'] as String?);
    final os = _normaliseVisualAcuity(r['os_snellen'] as String?);
    final ou = _normaliseVisualAcuity(r['ou_near_snellen'] as String?);
  ...
  Widget _pill(String eye, String value) {
    final isBad =
        value != '6/6' && value != '6/9' && value != _missingVaValue;
      ...
      list.add(
        _Patient(
          ...
          ageGroup: age < 18
              ...
              ? 'elderly'
              : 'adult',
          od: _normaliseVisualAcuity(r['od_snellen'] as String?),
          os: _normaliseVisualAcuity(r['os_snellen'] as String?),
          ou: _normaliseVisualAcuity(r['ou_near_snellen'] as String?),
          outcome: outcome,
          date: screeningDate != null
              ? _formatDate(screeningDate)
              : 'Not screened',
          id: r['id'] as String,
          phone: (r['phone'] as String?) ?? '',
          facility: (r['referral_facility'] as String?)?.let(
            (f) => f.isEmpty ? null : f,
          ),
          referralStatus: (r['referral_status'] as String?)?.let(
            (s) => s.isEmpty ? null : s,
          ),
...
class _CampaignPatientCardState extends State<_CampaignPatientCard> {
                ...
                onTap: () async {
                  ...
                  if (mounted)
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Status updated to ' + (st['label'] as String),
                          style: GoogleFonts.inter(
                            fontSize: 12,
                            color: Colors.white,
                          ),
  ...
  Future<void> _exportPatientDataCampaign(_Patient p) async {
    await _exportPatientReport(context, p);
  }

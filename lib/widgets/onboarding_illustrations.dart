...
class _ChwPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final smile = Path()
      ..moveTo(cx - 10, cy - 20)
      ..quadraticBezierTo(cx, cy - 13, cx + 10, cy - 20);
    canvas.drawPath(
      smile,
      stroke
        ..strokeWidth = 2.5
    ...
    final cap = Path()
      ..moveTo(cx - 30, cy - 30)
      ..arcTo(
        Rect.fromCenter(center: Offset(cx, cy - 30), width: 60, height: 60),
        pi,
        pi,
        false,
      )
...
class _PatientCarePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final clip = RRect.fromRectAndRadius(
      Rect.fromCenter(center: Offset(cx - 18, cy + 8), width: 150, height: 185),
    ...
    final smile = Path()
      ..moveTo(cx - 26, cy - 33)
      ..quadraticBezierTo(cx - 18, cy - 26, cx - 10, cy - 33);
    canvas.drawPath(
      smile,
      stroke
        ..strokeWidth = 2.2
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromCenter(center: Offset(cx - 18, cy - 10), width: 90, height: 8),
        const Radius.circular(4),
      ),
      fill2,
    );
    ...
    for (int i = 0; i < 2; i++) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(center: Offset(bx, cy + 100), width: 36, height: 20),
    ...
    final card = RRect.fromRectAndRadius(
      Rect.fromCenter(center: Offset.zero, width: 96, height: 66),
  ...
  bool shouldRepaint(_PatientCarePainter old) =>
      old.checkScale != checkScale || old.color != color;
}

// Activity dashboard
class ActivityIllustration extends StatefulWidget {
  const ActivityIllustration({super.key, this.color = Colors.white});
  final Color color;
  @override
  State<ActivityIllustration> createState() => _ActivityIllustrationState();
}

class _ActivityIllustrationState extends State<ActivityIllustration>
    with TickerProviderStateMixin {
...
class _ActivityPainter extends CustomPainter {
  const _ActivityPainter({required this.color, required this.growFactor});
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final frame = RRect.fromRectAndRadius(
      Rect.fromCenter(center: Offset(cx, cy), width: 220, height: 185),
      const Radius.circular(18),
    );
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(cx - 110, cy - 92, 220, 28),
        const Radius.circular(18),
      ),
      fill2,
    );
    ...
    final c1 = RRect.fromRectAndRadius(
      Rect.fromCenter(center: Offset.zero, width: 72, height: 38),
      const Radius.circular(12),
    );
    ...
    for (int r = 1; r <= 3; r++) {
      canvas.drawArc(
        Rect.fromCenter(center: Offset.zero, width: r * 16.0, height: r * 16.0),
        3.14159 + 3.14159 / 4,
        3.14159 / 2,
        false,
        Paint()
          ..color = color.withValues(alpha: 0.5 - r * 0.1)
          ..strokeWidth = 2.0
          ..style = PaintingStyle.stroke
          ..strokeCap = StrokeCap.round,
      );
  ...
  bool shouldRepaint(_ActivityPainter old) =>
      old.growFactor != growFactor || old.color != color;
}
